#ifndef SKSTATICCLASS_H
#define SKSTATICCLASS_H

#include "skInclude.h"

class CSkStaticClass : public QObject
{
public:
    CSkStaticClass();
	~CSkStaticClass();
    ...
    {
        return QFileDialog::getOpenFileName(NULL,tr("Select File"),QCoreApplication::applicationDirPath(),filter);
    }

    //获取选择文件集合
    static QStringList GetFileNames(QString filter)
    {
        return QFileDialog::getOpenFileNames(NULL,tr("Select File"),QCoreApplication::applicationDirPath(),filter);
    }
    ...
    static bool CopyFile(QString sourceFile, QString targetFile)
    {
        ...
        {
            int ret=QMessageBox::information(NULL,tr("Hint"),tr("File already exists, whether to replace?"),
	...
	//写入文件内容
	static bool writeFileContent(QString strFile,const QByteArray buf)
	{
		QFile file(strFile);
		QString txt;
		...
		{
			qint64 ret = file.write(buf);
			if (ret == -1 || ret != buf.size())
				return false;
		}
		file.close();
		return true;
	...
	static QString getFileMd5(QString filePath)
	{
		...
		{
			SWarning(QString(tr("Md5 file [%1] open error.")).arg(filePath),__FILE__,__LINE__);
			return "";
		}
